...
class TestRequestLoggingServer(unittest.TestCase):
    ...
    def test_static_response_rule_skips_upstream_forwarder(self) -> None:
    ...
    def test_static_response_rule_skips_editor_modifier(self) -> None:
        ...
        config = RuntimeTestContext()
        config.add_open_editor_rule("http://example.com/mock", method="GET")
        config.add_static_response_rule(
            url="http://example.com/mock",
            status_code=200,
            reason="OK",
    ...
    def test_streaming_chunked_response_is_sent_with_valid_client_framing(self) -> None:
        class ChunkedStream:
            status_code = 404
            reason = "Not Found"
            headers = {"Content-Type": "text/html", "Transfer-Encoding": "chunked"}

            def iter_body_chunks(self, _chunk_size: int):
                return iter((b"<html><body>not found</body></html>",))

            def close(self) -> None:
                pass

        class ChunkedForwarder:
            def forward(self, _request: ForwardRequest) -> ForwardResponse:
                raise AssertionError("Streaming path expected.")

            def open_stream(self, _request: ForwardRequest) -> ChunkedStream:
        ...
        self.server = create_server(
            ...
            0,
            runtime_context=self.runtime_context,
            forwarder=ChunkedForwarder(),  # type: ignore[arg-type]
        )
        self.thread = threading.Thread(target=self.server.serve_forever, daemon=True)
        self.thread.start()
        self.host, self.port = self.server.server_address

        status, data = self._request("GET", "http://example.test/missing", headers={"Host": "example.test"})

        self.assertEqual(status, 404)
        self.assertEqual(data, b"<html><body>not found</body></html>")
