...
from proxyscope.adapters.mitm.certificates import (
...
class TestMitmCertificateErrorPaths(unittest.TestCase):
    ...
    def test_read_openssl_output_returns_none_for_subprocess_errors(self) -> None:
        ...
        with patch("proxyscope.adapters.mitm.certificates.subprocess.run", side_effect=FileNotFoundError):
        ...
        with patch("proxyscope.adapters.mitm.certificates.subprocess.run", side_effect=failed):
            self.assertIsNone(authority._read_openssl_output(["x509"]))  # noqa: SLF001
    ...
    def test_run_openssl_raises_mitm_error_with_stderr(self) -> None:
        ...
        with patch("proxyscope.adapters.mitm.certificates.subprocess.run", side_effect=error):
