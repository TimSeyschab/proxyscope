...
from proxyscope.adapters.mitm.certificates import MitmCertificateError
...
from proxyscope.bootstrap.composition import (
...
class TestMitmComposition(unittest.TestCase):
    ...
    def test_create_mitm_interceptor_bootstraps_ca_when_enabled(self) -> None:
        ...
        with patch("proxyscope.bootstrap.composition.default_ca", return_value=fake_ca):
    ...
    def test_create_mitm_interceptor_skips_ca_bootstrap_when_disabled(self) -> None:
        with patch("proxyscope.bootstrap.composition.default_ca") as default_ca_mock:
            interceptor = create_mitm_interceptor(
                runtime_context=self.runtime_context,
                enabled=False,
                ca_root=None,
    ...
    def test_spa_profile_settings_create_a_mitm_interceptor(self) -> None:
        fake_ca = Mock()
        fake_ca.ensure_ca_material.return_value = False
        server = Mock()
        server.server_address = ("127.0.0.1", 8080)
        ...
        self.assertTrue(graph.settings.cache_invalidation_enabled)
