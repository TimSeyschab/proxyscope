...
from proxyscope.application.events import EventBus, ExchangeCompleted, RequestObserved, ResponseObserved
...
class TestRequestResponseLogging(unittest.TestCase):
    ...
    def test_recorder_publishes_request_response_and_completion_events(self) -> None:
        events: list[object] = []
        bus = EventBus()
        ...
        recorder.record_response(
            ExchangeResponse(200, "OK", {}, b"done"),
            request_id=request_id,
            duration_ms=1.0,
            client_ip="127.0.0.1",
            target_host="example.com",
        ...
        self.assertIsInstance(events[0], RequestObserved)
        self.assertIsInstance(events[1], ResponseObserved)
        self.assertIsInstance(events[-1], ExchangeCompleted)
