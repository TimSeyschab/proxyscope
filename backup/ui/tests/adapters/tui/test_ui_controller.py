...
from tests.support.components import core_component_manager
...
class TestRuntimeUIController(unittest.TestCase):
    def _controller(self, *, journal: RequestJournal | None = None) -> RuntimeCLI:
        ...
        services = runtime_application_services(config, request_journal=journal)
        manager = core_component_manager(services)
        return RuntimeCLI(
            application_services=services,
            command_registry=manager.command_registry,
            shortcut_registry=manager.shortcut_registry,
        )
    ...
    def test_runtime_controller_implements_ui_controller_protocol(self) -> None:
        config = RuntimeTestContext()
        services = runtime_application_services(config, request_journal=RequestJournal())
        manager = core_component_manager(services)
    ...
    def test_textual_bindings_are_generated_from_active_shortcuts(self) -> None:
        controller = self._controller()
        app = RuntimeTextualApp(controller)

        self.assertEqual(
            {binding.key for binding in app.BINDINGS},
            {shortcut.key for shortcut in controller.shortcut_registry.list_shortcuts()},
        )
        self.assertTrue(all(binding.priority for binding in app.BINDINGS))

    def test_textual_input_ignores_global_shortcuts(self) -> None:
        async def run_test() -> None:
            controller = self._controller()
            app = RuntimeTextualApp(controller)
