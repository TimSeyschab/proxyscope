...
class TestTextualShortcutParsing(unittest.TestCase):
    registered_keys = {"shift+1", "shift+2", "shift+m", "shift+t"}

    def test_shift_plus_letter_maps_to_shortcut_token(self) -> None:
        self.assertEqual(
            _shortcut_token_from_key_event(key="shift+m", character=None, registered_keys=self.registered_keys), "shift+m"
        )
        self.assertEqual(
            _shortcut_token_from_key_event(key="shift+t", character=None, registered_keys=self.registered_keys), "shift+t"
        )

    def test_shift_plus_number_maps_to_view_shortcut_token(self) -> None:
        self.assertEqual(
            _shortcut_token_from_key_event(key="shift+1", character=None, registered_keys=self.registered_keys), "shift+1"
        )
        self.assertEqual(
            _shortcut_token_from_key_event(key="!", character="!", registered_keys=self.registered_keys), "shift+1"
        )
        self.assertEqual(
            _shortcut_token_from_key_event(key="shift+2", character=None, registered_keys=self.registered_keys), "shift+2"
        )
        self.assertEqual(
            _shortcut_token_from_key_event(key='"', character='"', registered_keys=self.registered_keys), "shift+2"
        )
        self.assertEqual(
            _shortcut_token_from_key_event(key="@", character="@", registered_keys=self.registered_keys), "shift+2"
        )
    ...
    def test_uppercase_character_maps_to_shortcut_token(self) -> None:
        self.assertEqual(
            _shortcut_token_from_key_event(key="m", character="M", registered_keys=self.registered_keys), "shift+m"
        )
        self.assertEqual(
            _shortcut_token_from_key_event(key="M", character=None, registered_keys=self.registered_keys), "shift+m"
        )

    def test_lowercase_without_shift_is_not_treated_as_shortcut(self) -> None:
        self.assertIsNone(_shortcut_token_from_key_event(key="m", character="m", registered_keys=self.registered_keys))
