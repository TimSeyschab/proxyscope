...
def _runtime_cli(*, config: RuntimeTestContext | None = None, journal: RequestJournal | None = None) -> RuntimeCLI:
    ...
    services = runtime_application_services(context, request_journal=journal)
...
class TestRuntimeCLI(unittest.TestCase):
    ...
    def test_execute_quit_command_requests_shutdown(self) -> None:
        ...
        services = runtime_application_services(RuntimeTestContext())
        manager = core_component_manager(services, request_shutdown=shutdown)
        cli = RuntimeCLI(
            application_services=services,
            command_registry=manager.command_registry,
            shortcut_registry=manager.shortcut_registry,
