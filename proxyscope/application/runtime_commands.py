    ...
    def execute(
        ...
        *,
        ...
        handlers: dict[str, Callable[[], CommandExecutionResult]] = {
            "loglevel": lambda: self._handle_loglevel(parts),
            "whitelist": lambda: self._handle_whitelist(parts),
            "wl": lambda: self._handle_whitelist(parts),
            "cache": lambda: self._handle_cache(parts, on_cache_toggle=on_cache_toggle),
            "mitm": lambda: self._handle_mitm(parts),
            "config": lambda: self._handle_config(parts, on_cache_toggle=on_cache_toggle),
            "policy": lambda: self._handle_policy(parts, on_schedule_policy_edit=on_schedule_policy_edit),
            "pol": lambda: self._handle_policy(parts, on_schedule_policy_edit=on_schedule_policy_edit),
        }
