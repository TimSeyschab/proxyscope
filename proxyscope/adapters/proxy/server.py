...
from proxyscope.adapters.proxy.forwarding import UpstreamForwarder, resolve_target_url
from proxyscope.adapters.proxy.http_bridge import map_incoming_request, write_forward_response
from proxyscope.adapters.proxy.tunnel_registry import TunnelConnectionRegistry
from proxyscope.adapters.proxy.types import TunnelInterceptor
        ...
        *,
        ...
        try:
            ...
            if _is_chunked(headers):
                # urllib3 decodes HTTP chunk framing before yielding body chunks.
                # Stream the decoded body with connection-delimited framing instead.
                headers = {name: value for name, value in headers.items() if name.lower() != "transfer-encoding"}
...
def _is_chunked(headers: dict[str, str]) -> bool:
    return any(
        name.lower() == "transfer-encoding" and any(token.strip() == "chunked" for token in value.lower().split(","))
        for name, value in headers.items()
    )
